...
/*
This is the main step where our NET learns! in order to write this little
...
precise the NET with respect to some DATA. think of it as an adjustment routine.
So, according to that we need to verify what is the DATA that we want to approximate.
...
the only tool that enables to achieve such a thing called the GRADIENT. the latter is just
...
we claim that for some infinitesimal positive lambda>0, E( W + lambda*(-GRAD(W)) ) < E(W)
...
all the weights in our NET according to the new weight W_0. as a consequence, we get a
modified NET that acts better than the previous one on the DATA. of course, we repeat
this processes several times in order to perfect our NET with respect to DATA. suppose
we want to summarize all basic necessary steps described above and take into consideration
...
we can’t think at this moment about a structure to contain DATA. the reason
is quite obvious: we don’t have any data so far so we can’t think of how to
...
we would like to begin with the known XOR gate. this one supposed to be quite
simple as it contains only 4 lines of information. there're two variables x,y
...
which TOPOLOGY to choose. since there're two variables then the first layer has
...
GRAD:
since you want to differentiate functions for gradient purposes, the only way you
...
positive quantity h>0. this raises the question: which small quantity should we use?
this particular question made us to browse the internet and after careful examination
we have reached our desired candidate: DBL_EPSILON from float library is the least
